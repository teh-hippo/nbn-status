...
- Production runs on the Azure Functions **Flex Consumption** plan (FC1, Linux) in `australiaeast`. The Function App name is randomised (`nbn-status-604` at time of writing) and the URL is the default `*.azurewebsites.net`; ambiguity comes from the random name, not from a custom domain.
...
- All Azure resources are managed by the Terraform module under `infra/`. State lives in `nbnstatusstore/tfstate/nbn-status.tfstate` (azurerm backend, shared-key auth — `nbnstatusstore.allowSharedKeyAccess` must stay `true`).
...
- Production runs as Function App `nbn-status-604` (randomised name; the previous `nbn-status` was retired in the Flex Consumption migration) in resource group `nbn-status-rg`.
