...
- `backend.tf` — state in `nbnstatusstore/tfstate/nbn-status.tfstate`.
...
- Flex does not support App Service Managed Certificates, so the previous custom hostname (`nbn.totesnotexternal.xaz.lol`) is dropped and Microsoft's auto-supplied cert on the default `*.azurewebsites.net` is used instead.
- `nbnstatusstore.allowSharedKeyAccess` MUST stay `true`; the azurerm Terraform backend uses shared-key auth in this configuration.
...
(`nbn.totesnotexternal.xaz.lol`), and the custom hostname binding are added
to this module via further `import` blocks and immediately destroyed in the
same apply. The `nbn-status.azurewebsites.net` and
`nbn.totesnotexternal.xaz.lol` redirect URIs are then removed from the
Entra ID app reg.
